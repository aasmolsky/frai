...
After `git clone`, run setup **before** your first task — client integrations are not stored in git.

```bash
git clone git@github.com:org/my_llm_project.git my_project
cd my_project
cp .env.example .env          # required — .env is git-ignored
frai setup --claude           # CLI mode: MCPs + slash commands (pick your client)
# frai setup --codex
# frai setup --cursor
# frai setup --claude --cursor   # combine if you use more than one
frai check
frai exec AnalyzeItem::Task "query(hello world)"
```

**Do not skip `frai setup` after clone.** The repo has `mcp/*.rb` definitions, but Claude slash commands (`.claude/commands/`), Codex MCP registration, and `.cursor/mcp.json` are created locally by setup — they are not committed. Without setup, CLI/HITL mode will not see MCPs or task commands.

| Step | In git? | After clone |
|---|---|---|
| `mcp/*.rb`, tasks, agents | yes | ready |
| `.env` | no | `cp .env.example .env` and fill in |
| `.claude/commands/`, client MCP config | no | `frai setup --claude` / `--codex` / `--cursor` |

**API mode only** (automation, `LLM_MODEL` set): `frai setup` is optional — Frai connects MCPs at runtime. You still need `cp .env.example .env`.

Re-run `frai setup` when tasks, MCP servers, or `.env` change.
...
module AnalyzeItem
  class Task < BaseTask
    schema do
      ...
      use :guidelines                               # sub-directive
      run :fetch_context do                         # script
        input   type: String
...
| `use :name` | `schema do` | Sub-directive file |
| `run :name do` | `schema do` | Script declaration |
| `directive :name do` | top-level | **Optional.** Custom entry directive; default is `:task` → `task.md.erb` |

Default entry point is `directives/task.md.erb`. Declare `use` and `run` directly in `schema do`.
Use `directive :name do` only when the entry file has a different name — e.g. `directive :analyze` → `analyze.md.erb`.
Wrapping `use` / `run` in `directive :task do` is equivalent for the default entry.
...
Slash commands for tasks are created by `--claude` setup (not by `frai gt`). Class names are taken from the loaded task class (e.g. `PrepareLLMReport::Task`), not guessed from the folder name — re-run setup to update commands after renames.

```bash
frai setup --claude
# creates or updates .claude/commands/analyze_item.md for each task
```
...
| Slash commands / MCPs missing after `git clone` | Run `frai setup --claude` / `--codex` / `--cursor` — client config is not in the repo |
