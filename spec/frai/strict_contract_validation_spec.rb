...
RSpec.describe "Strict Contract Validation: use and run" do
  ...
  context "when sub-directive is not declared in schema" do
    it "raises Frai::UndeclaredDirective" do
      File.write(File.join(project_root, "tasks", "test_task", "directives", "main.md.erb"), "<%= use(:undeclared_sub) %>")
      ...
      allow_any_instance_of(Frai::DirectiveRenderer).to receive(:find_directive!).with(:main).and_return(File.join(project_root, "tasks", "test_task", "directives", "main.md.erb"))
  ...
  context "when script is not declared in schema" do
    it "raises Frai::UndeclaredScript" do
      File.write(File.join(project_root, "tasks", "test_task", "directives", "main.md.erb"), "<% run(:undeclared_script) %>")
